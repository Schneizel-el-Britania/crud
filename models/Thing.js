class Thing {
  ...
  static async updateByPk(valuePk, updateValues) {
    const formUpdateValuesString = (updateValues) => Object.entries(updateValues).map(([key, value]) => `"${key}"='${value}'`).join(',');
    const { rows } = await this.client.query(`
    UPDATE "${this.tableName}"
    SET ${formUpdateValuesString(updateValues)}, "updatedAt"='${new Date().toISOString()}'
    WHERE "id"=${valuePk}
    RETURNING *;
  `);
    return rows;
